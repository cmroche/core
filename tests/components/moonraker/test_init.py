...
async def test_setup_simple(hass: HomeAssistant) -> None:
    ...
    with patch(
        ...
        return_value=True,
    ) as sensor_setup, patch(
        "homeassistant.components.moonraker.binary_sensor.async_setup_entry",
        return_value=True,
    ) as binary_sensor_setup, patch(
        "homeassistant.components.moonraker.button.async_setup_entry",
    ) as button_setup, patch(
        "homeassistant.components.moonraker.switch.async_setup_entry",
    ) as switch_setup:
        entry = get_mock_entry(hass, ENTRY_NAME)
        await setup_entry(hass, entry)

        assert len(binary_sensor_setup.mock_calls) == 1
        assert len(button_setup.mock_calls) == 1
